...
async def settingsCommands(message, settings):
    ...
    if args[0] == "status":
        statusEmbed = discord.Embed(
            title=(f"Skybot {version}"),
            url=botInfo["githubLink"],
        ...
        if settings["enableBot"] == False:
            statusEmbed.add_field(
                name="Caution",
                value="The bot is currently disabled. To re-enable, type `$settings enableBot True` with moderator permissions",
            )
    ...
    if args[0].lower() == "settings":  # Call settings - Settings embed
        if len(args) == 1:
            messageEmbed = discord.Embed(
                title=(f"Server Settings for {message.channel.guild}")
            )
            ...
            for setting in settings:
                messageEmbed.add_field(
                    name=(f"{setting}"),
                    value=(
                        f'{settingDescriptions[setting]["description"]}\n**Value:** {settings[setting]}\n**Type:** {settingDescriptions[setting]["type"]}'
                    ),
                    inline=False,
                )
        ...
        if len(args) == 2:
            ...
            if (
                ...
                await message.channel.send(
                    "You do not have the permissions to change the settings"
                )
                return
            ...
            with open(
                (f"guildSettings/guilds/{message.channel.guild.id}.json"), "w"
            ) as f:
